...
exports_chained_dollars = data(strcmp(data.x_SeriesCode', 'A020RX'), :);

% Extract quarter and variable columns
period = exports_chained_dollars.Period;
A020RX = exports_chained_dollars.Value;

% Remove thousands separators and convert 'Value' to numeric
A020RX = strrep(A020RX, ',', '');
A020RX = str2double(A020RX);
...
figure;
plot(period, A020RX, '-o');
title('Quarterly exports for the USA, 1947-2024');
xlabel('Quarter');
ylabel('Exports');
grid on;

% Add source information below the graph
source_text = 'Source: NIPA, [A020RX-Q] Exports of goods and services, Chained Dollars, Level, as of 2 July 2024';
...
% Save the figure
saveas(gcf, 'outputs/exports_A020RX-Q.pdf');